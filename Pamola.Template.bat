...
dotnet new classlib -n Source
echo ^<Project Sdk^="Microsoft.NET.Sdk"^> >Source\Source.csproj
echo ^<PropertyGroup^> >>Source\Source.csproj
echo ^<TargetFramework^>netstandard2.0^</TargetFramework^> >>Source\Source.csproj
...
echo ^<TargetFramework^>netcoreapp2.1^</TargetFramework^> >>Tests\Tests.csproj 
...
echo       - image: microsoft/dotnet:2.2-sdk >>.circleci\config.yml 
